import { SetCalendarException } from '@delivery-os/application';
import { setCalendarExceptionCommandSchema } from '@delivery-os/contracts';
...
export async function PUT(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    const command = setCalendarExceptionCommandSchema.parse({
      ...body,
      schemaVersion: '1',
      workspaceId,
      projectId,
    ...
    return NextResponse.json(await new SetCalendarException(projectStore).execute(command));
  } catch (error) {
