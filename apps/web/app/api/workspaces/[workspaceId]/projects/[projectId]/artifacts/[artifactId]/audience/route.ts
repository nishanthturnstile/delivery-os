import { SetArtifactAudience } from '@delivery-os/application';
import { setArtifactAudienceCommandSchema } from '@delivery-os/contracts';
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    const command = setArtifactAudienceCommandSchema.parse({
      ...artifactEnvelope({
    ...
    return NextResponse.json(await new SetArtifactAudience(artifactStore).execute(command));
  } catch (error) {
