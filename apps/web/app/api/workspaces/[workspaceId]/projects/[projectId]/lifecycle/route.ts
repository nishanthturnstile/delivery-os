import { TransitionProjectLifecycle } from '@delivery-os/application';
import {
...
export async function GET(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    const requestedState = projectLifecycleStateSchema.parse(
      request.nextUrl.searchParams.get('requestedState'),
    );
    const [readiness, history] = await Promise.all([
      projectStore.getProjectReadiness(
        authContext.session.user.id,
        workspaceId,
        projectId,
        requestedState,
      ),
      projectStore.listProjectLifecycleHistory(authContext.session.user.id, workspaceId, projectId),
    ]);
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    const command = transitionProjectLifecycleCommandSchema.parse({
      ...body,
      schemaVersion: '1',
      workspaceId,
      projectId,
    ...
    return NextResponse.json(await new TransitionProjectLifecycle(projectStore).execute(command));
  } catch (error) {
    return apiError(error, correlationId);
  }
}
