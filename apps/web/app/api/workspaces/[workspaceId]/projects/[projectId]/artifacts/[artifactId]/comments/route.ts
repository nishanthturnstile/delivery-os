import { CreateArtifactComment } from '@delivery-os/application';
import { createArtifactCommentCommandSchema } from '@delivery-os/contracts';
...
export async function GET(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    return NextResponse.json({
      schemaVersion: '1',
      comments: await artifactStore.listComments(
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    correlationId = authContext.correlationId;
    const { workspaceId, projectId, artifactId } = await context.params;
    const command = createArtifactCommentCommandSchema.parse({
      ...artifactEnvelope({
    ...
    return NextResponse.json(await new CreateArtifactComment(artifactStore).execute(command), {
      status: 201,
    });
  } catch (error) {
    return apiError(error, correlationId);
